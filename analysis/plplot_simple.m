function h=plplot_simple(x, xmin, alpha)
% modified by EK % put in dummies for xmin and alpha
...
%         figure;
        h(1) = loglog(c(:,1),c(:,2),'.','MarkerSize',11,'MarkerFaceColor',[1 1 1]); hold on;
%         h(2) = loglog(cf(:,1),cf(:,2),'k--','LineWidth',2); hold off;
        ...
        ylabel('$Pr(A \geq a)$','FontSize',16);
        xlabel('Area ($km^2$)','FontSize',16)
