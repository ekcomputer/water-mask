...
plot(0*Acenters + Acenters(b),logspace(0,7,length(Acenters)),...
    'color','r', 'LineWidth', lw,'HandleVisibility','off');
h = get(gca,'children');
h = [h(end); h(1:end-1)];
set(gca,'children',h);
...
legend({'Power law fit', 'Truncated power law fit','Data'}, 'FontSize', 24);
